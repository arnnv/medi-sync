...
def reportPredict(dataset: List[Dict[str, str]]) -> None:
    ...
    part_precision = precision_score(y_true_parts, y_pred_parts, average='weighted', zero_division=1)
    part_recall = recall_score(y_true_parts, y_pred_parts, average='weighted', zero_division=1)
    part_f1 = f1_score(y_true_parts, y_pred_parts, average='weighted', zero_division=1)

    # Calculate metrics for fracture status
    status_accuracy = accuracy_score(y_true_fracture, y_pred_fracture)
    status_precision = precision_score(y_true_fracture, y_pred_fracture, average='binary', pos_label='fractured', zero_division=1)
    status_recall = recall_score(y_true_fracture, y_pred_fracture, average='binary', pos_label='fractured', zero_division=1)
    status_f1 = f1_score(y_true_fracture, y_pred_fracture, average='binary', pos_label='fractured', zero_division=1)
